...
@section('content')
  ...
  <h1  class="battles-title"><b>Batalla de Bandas:</b> Llave 3</h1>
    <div class="container">
      <div class="inner-container">
        ...
        <h1 class="favorite-select-battle" ><b>TUS 5 FAVORITAS</b></h1>
          <div class="my-right votes">
          <form class="form-register" action="{{ action('BattleController@thirdkey_votes') }}" method="post" role="form">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
              ...
              <select class="input-type-1 select-type-1 selectvotes" required="required" data-type="string" id="fav1" name="fav1">
               ...
               <select class="input-type-1 select-type-1 selectvotes" required="required" data-type="string" id="fav3" name="fav3">
                <option value disabled selected hidden>Seleccioná tu favorita</option>
                  @foreach($videos as $video)
                    <option class="select-purple" value="{{$video->id_video}}">{{$video->name_video}}</option>
                  @endforeach
              </select>
               <select class="input-type-1 select-type-1 selectvotes" required="required" data-type="string" id="fav4" name="fav4">
                <option value disabled selected hidden>Seleccioná tu favorita</option>
                  @foreach($videos as $video)
                    <option class="select-purple" value="{{$video->id_video}}">{{$video->name_video}}</option>
                  @endforeach
